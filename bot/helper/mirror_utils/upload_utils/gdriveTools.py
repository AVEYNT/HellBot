    ...
    def clone(self, link):
        ...
        try:
            ...
            if mime_type == self.__G_DRIVE_DIR_MIME_TYPE:
                ...
                buttons = ButtonMaker()
                durl = short_url(durl)
                buttons.buildbutton("🔷 Download", durl)
                if INDEX_URL is not None:
                    url_path = rquote(f'{meta.get("name")}')
                    url = f'{INDEX_URL}/{url_path}/'
                    url = short_url(url)
                    buttons.buildbutton("🔸 Index", url)
            else:
                ...
                if INDEX_URL is not None:
                    ...
                    if VIEW_LINK:
                        ...
                        urls = short_url(urls)
                        buttons.buildbutton("♦️ Stream", urls)
    ...
    def drive_list(self, fileName, stopDup=False, noMulti=False, isRecursive=True, itemType=""):
        ...
        for index, parent_id in enumerate(DRIVES_IDS):
            ...
            if not response["files"] and noMulti:
                ...
                continue
            if not Title:
                msg += f'<h4>Search Result For <i> {fileName} </i> </h4><br><br><b><a href="https://t.me/{CHANNEL_USERNAME}"> Over Modular </a></b> || <b><a href="https://t.me/OverModulerbot"> Owner </a></b> <br>'
                Title = True
